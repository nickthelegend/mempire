...
const FINAL = join(OUT, 'mempire-day-3.mp4');
const GAMEPLAY = join(HERE, 'assets', 'tour.mp4');

const run = (cmd, args) =>
  execFileSync(cmd, args, { cwd: HERE, stdio: 'inherit' });

if (!existsSync(GAMEPLAY)) {
  console.error(`missing ${GAMEPLAY} — capture it first with app/capture-tour.mjs`);
  process.exit(1);
}
