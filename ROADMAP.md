...
| 5 | Daily Shop, 4 rotating offers, paid reroll | 🔨 | M | Only route to a coin you don't hold. |
...
| 42 | Loading screen with art + progress | 🔨 | S | ~30MB of models currently pop in unannounced. |
| 43 | Compress models (draco/meshopt) | 🔨 | S | 46MB of GLB is too heavy for mobile. |
| 44 | First-run tutorial | ⬜ | M | Judges arrive cold. |
| 45 | Victory/defeat cinematics | ⬜ | M | Currently a panel; should be a moment. |
| 46 | Haptics on mobile | ⬜ | S | Cheap, and it lands. |
...
2. **#43 + #42** compress models and add a loading screen. The single biggest
   perceived-quality gap right now is a blank arena while 46MB downloads.
3. **#17 + #18** devnet deploy and wire the client. This converts "great demo"
   into "working product", which is the judging line.
4. **#5 Shop** — the highest revenue-per-hour feature still unbuilt.
5. **#7 sponsorship** — the pitch nobody else in the bracket can make.
